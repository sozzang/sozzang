...
class Member implements Comparable<Member> {
	...
	public Member() {
		...
		/*
		 * int num = 7; int result1 = (++num) - 5; // result1는 3, num은 8 int result2 =
		 * (num++) - 5; // result2는 2, num은 8
		 */
	}
	...
	public String toString() {
		return String.format("[%d] %s\t%.1fkg\t%.1fcm\n", memberID, name, weight, height); // prtinf 안 거쳐도 string에서 바로
																							// format 쓸 수 있다!
	}

	@Override
	public int compareTo(Member o) { // 헤더 int로 고정
	...
	private void printBMIList() {// BMI별로 출력하는 메소드
		...
		for (int i = 0; i < 10; i++) {
			if (m_BMI[i] >= 35) {
				...
				// ★else라고 하거나, else if (m_BMI[i] >= 0)라고 하면 m_BMI[i]가 0일때! 즉, m[i]가 null일때도
				// if중괄호 안으로 들어가게됨!!
				// ★null은 toString이 안되니까 NullPointerException에러가 뜸 -> m_BMI[i]가 0일때는
				// toString안걸리게 제외 해줘야함
